...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['select_id_proCopmposer_reduction']) && !empty($_POST['select_id_proCopmposer_reduction'])) {
    $idpro = $_POST['select_id_proCopmposer_reduction'];
        // Parcourir les ID des produits sélectionnés
        foreach ($idpro as $id) {
            // Vérifier si le produit a déjà ce coupon appliqué
            $test = "SELECT * FROM product_Composer_coupons WHERE components_id  = ? AND coupon_id = ?";
            ...
            if ($resul) {
                ...
                $sql_delete = "DELETE FROM product_Composer_coupons WHERE components_id = ? AND coupon_id = ?";
                ...
                $sql = "INSERT INTO product_Composer_coupons (components_id, coupon_id) VALUES (?, ?)";
            ...
            $datapro = "SELECT * FROM components  WHERE id = ?";
            $stmdata = $conn->prepare($datapro);
            $stmdata->execute([intval($id)]);
            $data = $stmdata->fetch();

            if (!$data) {
                header("Location: selection_pro.php?error=Erreur dans la récupération des données du Components ");
                exit;
            }
            ...
            $sqlappliquer = "SELECT c.*
                            FROM coupons c
                            INNER JOIN product_Composer_coupons  pc ON c.id = pc.coupon_id
                            WHERE pc.components_id  = ?
            ...
            if ($resulteappliquer) {
                ...
                if ($reduction > 90) {
                    header("Location: selection_pro.php?error=La réduction appliquée dépasse la limite logique pour le Components ID=$id");
                    exit;
                }
